...
type AppAction =
  | { type: 'DB_READY'; db: Database; totalPlays: number; selYear: string; selMonth: number; selKey: string }
  | { type: 'RESET' }
...
export function useFindRecord(): AnyRecord | null {
  ...
  const ds = state.gran === 'year'  ? state.db.year
           : state.gran === 'month' ? state.db.month
           : state.db.week;
