...
interface Progress { pct: number; message: string }
...
export default function UploadScreen() {
  ...
  return (
    <div className={styles.uploadScreen}>
      <h1 className={styles.uploadLogo}>
        Spotify <em>history</em>
      </h1>
      <p className={styles.uploadSub}>
        Upload your Spotify extended streaming history files<br />
        (<code>Streaming_History_Audio_*.json</code>) to explore your listening data.<br />
        Everything is processed locally — nothing leaves your device.
      </p>
        ...
        <ul className={styles.fileList} aria-label="Staged files">
          {pendingFiles.map(f => (
            <li key={f.name} className={styles.fileItem}>
              ...
              <span className={styles.fileItemSize}>{(f.size / 1024).toFixed(0)} KB</span>
      ...
      {pendingFiles.length > 0 && (
        ...
        >
          ...
          {isProcessing ? 'Processing' : `Process ${pendingFiles.length} file${pendingFiles.length !== 1 ? 's' : ''}`}
        </button>
      )}
